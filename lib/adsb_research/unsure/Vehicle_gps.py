# Feb 1, 2021
# This file is the vehicle class used in simulations where the data comes from actual flight logs
...
class VehicleGPS:
    ...
    @staticmethod
    def get_north_east_down(gps_coordinate):
        return pymap3d.geodetic2ned(gps_coordinate[0], gps_coordinate[1], gps_coordinate[2],
    ...
    def update_position(self, new_coordinate):
        self.position = self.get_north_east_down(new_coordinate)
        self.latitude_lsbs = self.encode_lsbs(new_coordinate[0])
        self.longitude_lsbs = self.encode_lsbs(new_coordinate[1])
        # self.latitude_lsbs, self.longitude_lsbs, altitude = self.extract_lsbs(new_coordinate)
