...
# Class that calculates all mso values for each vehicle
class UAT:
    def __init__(self, frame, previousRandomNumber):
        self.frame = frame
        self.previous_random_number = previousRandomNumber
        self.frame_before_restricted = 0
        self.message_start_opportunity = 0
        self.current_lsb_lat = 0
        self.current_lsb_long = 0
    ...
    def extract_lsbs(self, position):
        ...
        self.current_lsb_lat = self.encode_lsbs(coordinate[0])
        self.current_lsb_long = self.encode_lsbs(coordinate[1])
        return self.current_lsb_lat, self.current_lsb_long, coordinate[2]
    ...
    def get_least_significant_bits(self, frame):
        if frame == 0:
            return self.current_lsb_lat  # latitude
        elif frame == 1:
            return self.current_lsb_long  # longitude
        else:
