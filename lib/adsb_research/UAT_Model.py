...
class UAT_Model:
    ...
    def propagate(self):
        ...
        #self.num_mso_total += self.num_vehicles
        for j in range(self.num_vehicles):  # for each vehicle
            ...
            for k in range(self.vehicles[j].vehicle_identifier + 1, self.vehicles[j].vehicle_identifier + self.num_vehicles):
                #print("Number of runs: ",(self.vehicles[j].vehicle_identifier + 1) - (self.vehicles[j].vehicle_identifier + self.num_vehicles))
        ...
        # print("Before Sort: ")
        # for k in range(len(sorted_msos)):
        #     print(sorted_msos[k].message_start_opportunity)
        #print("After Sort: ")
            ...
            # Figure out which vehicles
                ...
                for k in range(self.num_vehicles):  # for each vehicle
                    if self.vehicles[k].message_start_opportunity == j:  # if the MSO matches the one of the collision
                        ...
                        #print("mso: ",j)
                        #print("vehicle msos: ",self.vehicles[k].message_start_opportunity)
                        #print("vehicle: ",k)
                ...
                if vehicles_involved: #if anything is in the list
        ...
        #print("Collisions len: ", len(collision_objects))
                        ...
                        # in power is not great enough for the closest to be selected, so all messages fail
                            ...
                            for ell in range(len(transmissions)):  # make them all False
                                if transmissions[ell]["successfuldecode"]:
                                    #print("Active 3")
                                    #for il in range(len(transmissions)):
                                    #    print(transmissions[il]["successfuldecode"],transmissions[il]["receiverID"])
    ...
    def printResults(self):
        ...
        print(" Collisions: ", self.collisionsPrint)
        print("MTL: ",self.MTL)
        print("MSO_total: ",totalMso)
        #with open('UAT_Monet_carlo Test.csv', 'a', newline='') as csvfile: #after first run change to a
        ...
        #    writer.writerow([str(self.num_vehicles),str(ERP_TYPE),str(self.Radius),str(self.maxSpeed),str(self.overlap),
        #            str(self.chance),str(self.closeness),str(self.collisionsPrint),str(self.MTL),str(self.false_from_high_power),str(self.seed)])
        #self.graph()
        #self.graphAni()
...
###############################################################################################
    def graph(self):
        ...
        #print("seconds: ",p.num_seconds)
        #print("seconds 2: ",self.numSeconds)
        #print("seconds 3: ",int(self.numSeconds))

        #print("total: ",len(self.totalCollisions))
        ...
        for j in range(self.num_vehicles):
            for i in range(0,int(self.numSeconds)):
                if(i == 1):
                    ...
                    # print("first")
                    #print("vel: ",len(self.vehiclePositions[i]))
                    # print("vel j: ",len(self.vehiclePositions[j][i]))
                    print("i: ",i)
                    print("j: ",j)
                    print("one:",self.vehiclePositions[j][i][0])
                    print("second")
                    print("two:",self.vehiclePositions[j][i][1])
                    # print("third")
                    collisions.append(ax.scatter(self.vehiclePositions[j][i][0],
                                                self.vehiclePositions[j][i][1],
                                                marker=".", c=color[j],s=5))
