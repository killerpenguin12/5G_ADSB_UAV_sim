            ...
            if (mso_to_check < (mso - 9)) or \
                ...
                if transmissions[0]["received_power"] >= p.receiver_min_trigger_level:
                    ...
                    if transmissions[0]["received_power"] - transmissions[1]["received_power"] >= \
                            3000:  # TODO different from main_sim_collision_enhanced.py
...
def main():
    ...
    for i in range(p.num_seconds):
        ...
        false_from_message_overlap += evaluate_consecutive_msos(vehicles)  # TODO different from main_sim_message_enhanced.py
    ...
    with open('JonOld.csv', 'a', newline='') as csvfile: #after first run change to a
        writer = csv.writer(csvfile, delimiter=',')            
        #writer.writerow(["Num Vehicles","Watts","Collision Radius","Max Speed","Total MSO Collisions", "Phys Crash","% MSO collsions/MSO" 
        #,"Zone Violated","Total MSO","overlap","chance","closeness","collisions","MTL","High Power"])
        writer.writerow([str(p.num_vehicles),str(ERP_TYPE),str(0),str(0),
        str(num_MSO_collisions),str(0),str(num_MSO_collisions/totalMso),
            str(0),str(totalMso),str(false_from_message_overlap),
                    str(false_by_chance),str(false_from_closeness),
                        str(false_from_collision),str(false_from_minimum_trigger_level),
                        (str(false_from_high_power ))])
