...
import random
#import openLogFiles as logs #apparently this is not used anymore?
...
def get_distance(vehicles, j, k):
    return int(np.ceil(np.sqrt((vehicles[j].position[0] - vehicles[k % p.num_vehicles].position[0]) ** 2.0 +
...
def main():
    ...
    init_vehicles(vehicles) #this function inits all the vehicles we have. From p.num_vehicles
    ...
    for i in range(p.num_seconds):
        ...
        for j in range(p.num_vehicles):
            vehicles[j].update_position(p.vehicle_positions[j][i]) #Do we have to include the movement of the UAV? Or do we have
            #a track. Also if we just input a position and how it changes we need to overwrite that if we want it to dodge.
            #if(get_distance(vehicles,j,i) < 1): #check if we are too close and need to move
            #    print("Ouch!")
            if i == 0:
    ...
    ### The following three lines are for memory usage ###
    ...
    #          north_vals.append(positions[i][j][0])
    #          east_vals.append(positions[i][j][1])
    #          down_vals.append(-positions[i][j][2])
    ...
    #             for k in range(len(overall_collision_objects[i][j]["vehicles_involved"])):
    #                 vehicle_id = overall_collision_objects[i][j]["vehicles_involved"][k].vehicle_identifier
    #                 ax.scatter(positions[vehicle_id][i][0],
    #                             positions[vehicle_id][i][1],
    #                             -positions[vehicle_id][i][2],
