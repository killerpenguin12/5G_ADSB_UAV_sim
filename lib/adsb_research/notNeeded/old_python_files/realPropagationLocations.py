...
for i in range(p.num_vehicles):
    ...
    uas = vehicle(positions[i][0], random.randrange(3601), i)  # FIXME indexing
    vehicles.append(uas)
...
for i in range(p.num_seconds):
    if i % p.one_percent == 0 and i:
        ...
        print(txt.format(percentage, current_time))
            ...
            for l in range(p.num_vehicles):
                if vehicles[l].MSO == consecutive_MSOs[j][k] and ((k % 2) != 0):
                    for m in range(p.num_vehicles):
                        if vehicles[l].transmissions[m] != 0:
                            if vehicles[l].transmissions[m].successful_decode:
                                vehicles[l].transmissions[m].successful_decode = False
        ...
        for k in range(p.num_vehicles):  # for every vehicle...
            ...
            if ((MSO_to_check < (MSO - 8)) or (
                ...
                for l in range(len(collision_objects[
                    ...
                    transmissions.append(vehicles[collision_objects[j].vehicles_involved[l]].transmissions[k])
