...
# This is the sim file used when the positional data comes from flight logs.
...
def init_vehicles(vehicles):
    for i in range(p.num_vehicles):
        # uas = VehicleGPS((0, 0, 0), random.randrange(3601), i) # random
        # uas = VehicleGPS((0, 0, 0), 1, i)  # both odd
        # uas = VehicleGPS((0, 0, 0), 2, i)  # both even
        # uas = VehicleGPS((0, 0, 0), i + 1, i)  # odd-even
        uas = VehicleGPS((0, 0, 0), i + 2, i)  # even-odd
        vehicles.append(uas)
...
def main():
    ...
    for i in range(p.num_seconds):
        ...
        for j in range(p.num_vehicles):
            ...
            if p.vehicle_positions[j][i][1] == -111.675520:
                print(p.vehicle_positions[j][i])
                print(vehicles[j].latitude_lsbs)
                print(vehicles[j].longitude_lsbs)
                print(mso)
    ...
    ax.scatter(p.vehicle_positions[0][0][1], p.vehicle_positions[0][0][0] + 0.01, c="white", s=point_size)
    ax.scatter(p.vehicle_positions[0][0][1], p.vehicle_positions[0][0][0] - 0.01, c="white", s=point_size)
    if plot_collisions:
