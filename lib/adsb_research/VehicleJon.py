...
class Vehicle:
    def __init__(self, position, frame=0, vehicle_identifier=0):
        ...
        self.update_position(position)  # this performs the tasks necessary to update position
        ...
        self.vehicle_identifier = vehicle_identifier  # this will just be an integer ID number
        self.transmissions = []  # this will hold transmission dicts
        #here I am going to make some variables that will hold the tranmission objects. These will be 
        #long lists which then will corespond to each vehicle I think. Doing this with a new vehicle and 
        #UAT model.
    ...
    def get_least_significant_bits(self, frame):
        # Due to the way this function is called, the frame value here will always be 0 or 1
        if frame == 0:
            return self.latitude_lsbs  # return latitude if 0
        elif frame == 1:
            return self.longitude_lsbs  # return longitude if 1
        else:
    ...
    def pseudo_random_number(self, frame):
        if frame == 0:  # The calculation is different for the first frame
    ...
    # See MSO details on pdf page 115 of DO-B282 (page 85)
    ...
    # I figured this out with DO-282B and a ton of trial and error
    @staticmethod
    def encode_lsbs(value):
        binary_value = value
        # negative values (South for LAT, West for LON) have slightly different encoding than positive
        if binary_value < 0:
            binary_value += 180
    ...
    # This takes a NED coordinate, converts it to geodetic, then extracts the LSBs for latitude and longitude
    ...
    def update_position(self, new_position):
        self.position = new_position  # update position member variable
        self.latitude_lsbs, self.longitude_lsbs, altitude = self.extract_lsbs(new_position)  # update LSBs and altitude
